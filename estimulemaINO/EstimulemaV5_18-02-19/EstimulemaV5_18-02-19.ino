...
const int RELAY_CH_2 = 7;            // Relay for CH1

const int Pin_Sync_Data = 8;         // Start and End data capture for accelerometer

const int BuzzerPin = 9;             // Buzzer pin activation for status messages
const int DAC_SS_PIN = 10;
const int Pin_signal_Control = 14;   // Enable data capture
const int med_out_signal1 = 15;      // for measure the output estimulation for ch1
const int med_out_signal2 = 16;      // for measure the output estimulation for ch2
const int Pin_Inter = 23;            // Emergency Button

// Original values for DAC conversion 
const int CH1_MAX_POS = 1883;
const int CH1_MIN_NEG = 1842;
const int CH2_MAX_POS = 2047;
const int CH2_MIN_NEG = 2047;
...
unsigned long one_freq = 0, min_t = 60e6; // Time multiplicator for minutes to us
...
struct stimulation_parameters{
  ...
  float ri; // in us
  float rf; // in us
  int ma; // Hz converted on Period T in us
...
void setup() {
  ...
  Serial1.println("1>0>");				  // Stop accel capture
  ...
  attachInterrupt(Pin_Inter, Stop_functions, LOW); // Extern interruption configuration
...
void split_functionality(){
  ...
  if(rh && s_c){
    ...
    Serial1.println("1>1>");		// Inicia captura de acel
  ...
  if(cr && s_c){ /// calibracion se sensor de aceleracion
    // Inicial Beep
    beep(1, 150);
    zeroChannels();
  ...
  if(data_sp[0].ch_act && data_sp[1].ch_act == 0 && s_c){
    ...
    // Bipp for start stimulation
    beep(1, 100);

    /**************** Importante para tener por lo menos 1s antes de estimular y poder capturar datos *****************/
    // Wait 2 seconds before start stimulation
    delay(1000);

    stimulation_training(1,0);

    digitalWrite(Pin_Sync_Data, 0); // Start data sync
    Serial1.println("1>0>");
    ...
    /**************** Importante para tener por lo menos 1s antes de estimular y poder capturar datos *****************/
    // Wait 1 seconds before start stimulation
    delay(1000);
...
void stimulation_training(int ch1, int ch2){
  ...
  while(ts_micros < t_stop){
    ...
    // Verify the time for every minute //-- esta opcion fue quitada porque atrasa el tiempo en la señal
  ...
  // End character for identify finish or stop stimulation in accel capture
  //Serial1.println("3>0>");
...
void Stop_functions() {
  ...
  Serial1.println("1>0>"); 				  // para capptura de acel
